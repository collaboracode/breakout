...
export function clearBricks(brickArray, width, height) {


  for (let row = 0; row < height; row++) {
    for (let col = 0; col < width; col++) {

      brickArray[row][col] = -1;
    }
  ...
  if ((bArr.length * (brickHeight + gap)) >= ball.y) {
    ...
    if (row >= 0 && col >= 0 && row <= bArr.length && col <= bArr[0].length) {
      if (bArr[row][col] !== -1) {
        ...
        const brick = getBrick(row, col, canvasWidth, canvasHeight, bArr)
        ballIsColliding = isColliding(ball, brick)
        side = whichSide(ball, brick);

        if (ballIsColliding && side) {
          bArr[row][col] = -1;
        }
        reflect = true;
      }
